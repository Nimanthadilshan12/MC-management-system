...
$form_details_data = isset($_SESSION['form_details_data']) ? $_SESSION['form_details_data'] : null;
    ...
    if (empty($errors)) {
        try {
            ...
            $_SESSION['submitted_data'] = [
                ...
                'medical_officer_signature' => $medical_officer_signature,
                'submission_date' => $submission_date,
                'form_details_data' => $form_details_data // Include form_details_data for PDF
            ];

            $success = "Form submitted successfully! Data saved to database.";
            unset($_SESSION['form_details_data']); // Clear form_details_data after submission
...
<head>
    ...
    <style>
        ...
        * {
        ...
        .certificate-number, .form-details {
        ...
        .form-details.show {
            display: block; /* Show when button is clicked */
        }
...
<body>
    ...
    <?php if ($form_details_data && $_SESSION['role'] === 'Doctor'): ?>
        <button class="view-details-button" onclick="toggleDetails()">View Submitted Patient Details</button>
        <div class="form-details" id="form-details">
            <h3>Submitted Patient Details</h3>
            <p><strong>Form Type:</strong> <?php echo htmlspecialchars($form_details_data['form_type'] === 'exam' ? 'Examinations' : 'Lectures/Practical Classes'); ?></p>
            <p><strong>Name:</strong> <?php echo htmlspecialchars($form_details_data['name']); ?></p>
            <p><strong>Address:</strong> <?php echo htmlspecialchars($form_details_data['address']); ?></p>
            <p><strong>Contact Number:</strong> <?php echo htmlspecialchars($form_details_data['contact_no']); ?></p>
            <p><strong>Registration Number:</strong> <?php echo htmlspecialchars($form_details_data['reg_no']); ?></p>
            <?php if ($form_details_data['form_type'] === 'exam'): ?>
                <p><strong>Year:</strong> <?php echo htmlspecialchars($form_details_data['year']); ?></p>
                <p><strong>Level:</strong> <?php echo htmlspecialchars($form_details_data['level']); ?></p>
                <p><strong>Semester:</strong> <?php echo htmlspecialchars($form_details_data['semester']); ?></p>
                <p><strong>Degree Programme:</strong> <?php echo htmlspecialchars($form_details_data['degree_programme']); ?></p>
            <?php else: ?>
                <p><strong>Academic Year:</strong> <?php echo htmlspecialchars($form_details_data['academic_year']); ?></p>
                <p><strong>Level:</strong> <?php echo htmlspecialchars($form_details_data['level']); ?></p>
                <p><strong>Semester and Year:</strong> <?php echo htmlspecialchars($form_details_data['semester_year']); ?></p>
            <?php endif; ?>
            <p><strong>Certificate Details:</strong> <?php echo htmlspecialchars($form_details_data['certificate_details']); ?></p>
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($form_details_data['subjects'] as $subject): ?>
                        <tr>
                            <td><?php echo htmlspecialchars($subject['name']); ?></td>
                            <td><?php echo htmlspecialchars($subject['code']); ?></td>
                            <td><?php echo htmlspecialchars($subject['dates']); ?></td>
                            <td><?php echo htmlspecialchars($subject['place']); ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
    ...
    <form method="POST" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        <div class="form-group">
            ...
            <input type="text" id="patient_name" name="patient_name" value="<?php echo htmlspecialchars($form_details_data['name'] ?? $patient_name); ?>" required>
        </div>
        <div class="form-group">
            <label for="address">Usual Address of Residence *</label>
            <textarea id="address" name="address" rows="3" required><?php echo htmlspecialchars($form_details_data['address'] ?? $address); ?></textarea>
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
            ...
            <select id="fit_for_duty" name="fit_for_duty" required>
    ...
    <script>
        try {
            ...
            function toggleDetails() {
                ...
                const button = document.querySelector('.view-details-button');
                if (detailsDiv.classList.contains('show')) {
                    detailsDiv.classList.remove('show');
                    button.textContent = 'View Submitted Patient Details';
                } else {
                    detailsDiv.classList.add('show');
                    button.textContent = 'Hide Submitted Patient Details';
                }
            }
            ...
            function generatePDF() {
                try {
                    ...
                    const formDetailsData = <?php echo json_encode($form_details_data); ?>;
                    if (data) {
                        ...
                        y += 10;
                        if (formDetailsData) {
                            doc.text(`Form Type: ${formDetailsData.form_type === 'exam' ? 'Examinations' : 'Lectures/Practical Classes'}`, 10, y);
                            y += 10;
                            if (formDetailsData.form_type === 'exam') {
                                doc.text(`Year: ${formDetailsData.year}`, 10, y);
                                y += 10;
                                doc.text(`Level: ${formDetailsData.level}`, 10, y);
                                y += 10;
                                doc.text(`Semester: ${formDetailsData.semester}`, 10, y);
                                y += 10;
                                doc.text(`Degree Programme: ${formDetailsData.degree_programme}`, 10, y);
                                y += 10;
                            } else {
                                doc.text(`Academic Year: ${formDetailsData.academic_year}`, 10, y);
                                y += 10;
                                doc.text(`Level: ${formDetailsData.level}`, 10, y);
                                y += 10;
                                doc.text(`Semester and Year: ${formDetailsData.semester_year}`, 10, y);
                                y += 10;
                            }
                            doc.text(`Certificate Details: ${formDetailsData.certificate_details}`, 10, y, { maxWidth: 190 });
                            y += 15;
                            doc.text('Subjects:', 10, y);
                            y += 10;
                            if (formDetailsData.subjects && Array.isArray(formDetailsData.subjects)) {
                                formDetailsData.subjects.forEach(subject => {
                                    doc.text(`- ${subject.name} (${subject.code}), Dates: ${subject.dates}, Place: ${subject.place}`, 10, y, { maxWidth: 190 });
                                    y += 10;
                                });
                            }
                        }
